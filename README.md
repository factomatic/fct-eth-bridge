...
without an intermediary. To the best of our knowledge, this is the first publickly available demonstration of trustless
interoperability between the Factom and Ethereum blockchains.

**This is proof a concept implementation, which has NOT undergone any formal securty audit and is NOT suitable for use
...
* `templates/`: contains a JS template for calling the smart contract, which is populated by the
`python/generate_claim_eth_params.py` helper script
...
maximum within 2 hours of the booking. No other party is allowed to book the same request within that timeframe.
...
of the last 1000 `DirectoryBlock KeyMR`s (MR=Merkle Root) -- a.k.a. the `WindowMR` -- of the Factom blockchain is
...
contract and it contains the Merkle roots of the previous 1000 `DirectoryBlock`s.
...
* the low-level transaction Factoid tx details, such as inputs, outputs, amounts and
RCDs
* the Merkle authentication path starting from the Factoid tx hash as a left all the way up to the relevant `WindowMR`
...
enough to make sense even from small value exchanges.
...
that older versions work as well. This code has been tested on UNIX-like operating system and very likely DOES NOT work
...
* Buying pFCT with ETH directly
    - since burning of FCT is just a Factoid tx, the same mechanism can be used that a transaction was done, which burns
    FCT to a given address. This would require a change to PegNet, which allows burning to a different FCT address.
* Factom as data layer for Ethereum
    ...
    smart contract similar to the one above for storing data on the Factom blockchain. The data is communicated
    off-chain and a deposit is made to the Ethereum smart contract containing the hash of the data + the amount of ETH
    that the user is willing to pay for recording that data on Factom. Once the data is recorded, the entity that
    recorded it, can sumit a Proof-of-Existence using the respective Factom entry and its Merkle authentication path.
...
A cryptographically more sound approach would be to require the account claiming the funds to produce a signature from
with the private key of the FCT address, which was used to make the transaction satisfying the request. However, Factom
uses the ed25519 signature scheme, while Ethereum currently uses EDCDSA over the secp256k1 curve. This means that the
EVM precompiles for signature verification cannot be used and doing an ed25519 inside the smart contract is
